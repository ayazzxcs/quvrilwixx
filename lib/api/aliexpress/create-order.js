...
async function getAliExpressConnectionForShop(shop) {
  const shopDomain = clean(shop);
  ...
  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/aliexpress_connections?shop_domain=eq.${encodeURIComponent(shopDomain)}&select=*&order=updated_at.desc&limit=1`,
    {
      headers: {
...
async function createAliExpressOrder(record, connection) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const shopDomain = clean(record.shop_domain);

    if (!shopDomain) {
      await updateFulfillmentRecord(fulfillmentId, {
        status: 'auto_failed_aliexpress_not_connected',
        failure_reason: 'Missing shop_domain on fulfillment record'
      });

      return res.status(400).json({
        ok: false,
        status: 'auto_failed_aliexpress_not_connected',
        error: 'Missing shop_domain on fulfillment record'
      });
    }

    const aliConnection = await getAliExpressConnectionForShop(shopDomain);

    if (!aliConnection) {
      await updateFulfillmentRecord(fulfillmentId, {
        status: 'auto_failed_aliexpress_not_connected',
        failure_reason: 'AliExpress account is not connected for this Shopify store'
      });

      return res.status(400).json({
        ok: false,
        status: 'auto_failed_aliexpress_not_connected',
        error: 'AliExpress account is not connected for this Shopify store'
      });
    }
    ...
    const aliResponse = await createAliExpressOrder(record, aliConnection);
