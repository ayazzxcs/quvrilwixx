...
module.exports = async function handler(req, res) {
  try {
    ...
    const productId =
      getRequestValue(req, 'productId') ||
      getRequestValue(req, 'pid');

    const productSku =
      getRequestValue(req, 'productSku') ||
      getRequestValue(req, 'sku');

    const variantSku =
      getRequestValue(req, 'variantSku');

    const countryCode =
      getRequestValue(req, 'countryCode', 'US');
    ...
    return res.status(200).json({
      ok: true,
      product: {
        pid: String(product.pid || productId || ''),
