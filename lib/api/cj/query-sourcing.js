...
async function queryCJWithFallbacks({ connection, cjSourcingId, cjSourceNumber }) {
  ...
  for (let index = 0; index < attempts.length; index++) {
    ...
    if (isRateLimitResponse(result.json)) {
      await sleep(1600);
      continue;
    }
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (!result || !isGoodCJResponse(result)) {
      const errorJson = result?.json || 'CJ query failed';

      if (isRateLimitResponse(errorJson)) {
        return res.status(429).json({
          ok: false,
          code: 'CJ_RATE_LIMITED',
          error: 'CJ rate limit reached. Please wait a few seconds and try again.',
          response: errorJson
        });
      }
