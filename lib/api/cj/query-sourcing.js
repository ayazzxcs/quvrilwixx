...
async function updateSourcingRequest({ shop, cjSourcingId, json }) {
  ...
  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/cj_sourcing_requests?shop_domain=eq.${encodeURIComponent(shop)}&cj_sourcing_id=eq.${encodeURIComponent(cjSourcingId)}`,
    {
      method: 'PATCH',
      headers: {
        apikey: SUPABASE_SERVICE_ROLE_KEY,
...
module.exports = async function handler(req, res) {
  try {
    ...
    try {
      ...
      throw new Error(
        'CJ sourcing query response was not JSON: ' +
          (text || '[EMPTY RESPONSE]')
      );
    }
