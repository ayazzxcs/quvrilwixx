const crypto = require('crypto');

const SUPABASE_URL = process.env.SUPABASE_URL;
const SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY;

const CJ_CREATE_SOURCING_URL =
  'https://developers.cjdropshipping.com/api2.0/v1/product/sourcing/create';

function clean(value) {
  return String(value || '').replace(/\s+/g, ' ').trim();
}

function safeUrl(value) {
  const raw = clean(value);

  if (!raw) return '';

  try {
    return new URL(raw).href;
  } catch {
    return raw;
  }
}

function tokenHash(value) {
  return crypto
    .createHash('sha256')
    .update(String(value || ''))
    .digest('hex');
}

function getRequestValue(req, key, fallback = '') {
  const body = req.body || {};
  const query = req.query || {};

  return (
    body[key] ||
    query[key] ||
    query[key.toLowerCase()] ||
    query[key.toUpperCase()] ||
    fallback
  );
}

function deepFindFirst(obj, keys) {
  const wanted = keys.map((key) => key.toLowerCase());
  const seen = new Set();

  function visit(value) {
    if (!value || typeof value !== 'object') return '';

    if (seen.has(value)) return '';
    seen.add(value);

    if (Array.isArray(value)) {
      for (const item of value) {
        const found = visit(item);
        if (found) return found;
      }

      return '';
    }

    for (const [key, item] of Object.entries(value)) {
      if (wanted.includes(String(key).toLowerCase())) {
        const cleaned = clean(item);
        if (cleaned) return cleaned;
      }
    }

    for (const item of Object.values(value)) {
      const found = visit(item);
      if (found) return found;
    }

    return '';
  }

  return visit(obj);
}

async function getShopifyStore(shop, installToken) {
  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/shopify_stores?shop_domain=eq.${encodeURIComponent(shop)}&select=*`,
    {
      headers: {
        apikey: SUPABASE_SERVICE_ROLE_KEY,
        Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`
      }
    }
  );

  const rows = await response.json();

  if (!response.ok || !Array.isArray(rows) || !rows.length) {
    return null;
  }

  const store = rows[0];

  if (store.install_token_hash !== tokenHash(installToken)) {
    return null;
  }

  return store;
}

async function getLatestCJConnectionForShop(shop) {
  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/cj_connections?shop_domain=eq.${encodeURIComponent(shop)}&select=*&order=updated_at.desc&limit=1`,
    {
      headers: {
        apikey: SUPABASE_SERVICE_ROLE_KEY,
        Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`
      }
    }
  );

  const rows = await response.json();

  if (!response.ok || !Array.isArray(rows) || !rows.length || !rows[0].access_token) {
    return null;
  }

  return rows[0];
}

function extractSourcingId(json) {
  return deepFindFirst(json, [
    'cjSourcingId',
    'cj_sourcing_id',
    'sourceId',
    'source_id',
    'sourcingId',
    'sourcing_id',
    'sourceIdStr'
  ]);
}

function extractSourceNumber(json) {
  return deepFindFirst(json, [
    'sourceNumber',
    'source_number',
    'sourceNo',
    'source_no',
    'cjSourceNumber',
    'cj_source_number',
    'sourcingNumber',
    'sourcing_number'
  ]);
}

async function findExistingByQuvirlProductId(shop, quvirlProductId) {
  if (!shop || !quvirlProductId) return null;

  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/cj_sourcing_requests?shop_domain=eq.${encodeURIComponent(shop)}&quvirl_product_id=eq.${encodeURIComponent(quvirlProductId)}&select=*&order=updated_at.desc&limit=1`,
    {
      headers: {
        apikey: SUPABASE_SERVICE_ROLE_KEY,
        Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`
      }
    }
  );

  const rows = await response.json();

  if (!response.ok || !Array.isArray(rows) || !rows.length) {
    return null;
  }

  return rows[0];
}

async function findExistingByProductUrl(shop, productUrl) {
  if (!shop || !productUrl) return null;

  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/cj_sourcing_requests?shop_domain=eq.${encodeURIComponent(shop)}&product_url=eq.${encodeURIComponent(productUrl)}&select=*&order=updated_at.desc&limit=1`,
    {
      headers: {
        apikey: SUPABASE_SERVICE_ROLE_KEY,
        Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`
      }
    }
  );

  const rows = await response.json();

  if (!response.ok || !Array.isArray(rows) || !rows.length) {
    return null;
  }

  return rows[0];
}

async function findExistingByImageAndTitle(shop, productImage, productTitle) {
  if (!shop || !productImage || !productTitle) return null;

  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/cj_sourcing_requests?shop_domain=eq.${encodeURIComponent(shop)}&product_image_url=eq.${encodeURIComponent(productImage)}&product_title=eq.${encodeURIComponent(productTitle)}&select=*&order=updated_at.desc&limit=1`,
    {
      headers: {
        apikey: SUPABASE_SERVICE_ROLE_KEY,
        Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`
      }
    }
  );

  const rows = await response.json();

  if (!response.ok || !Array.isArray(rows) || !rows.length) {
    return null;
  }

  return rows[0];
}

async function findExistingSourcingRequest({
  shop,
  quvirlProductId,
  productUrl,
  productImage,
  productTitle
}) {
  const byProductId = await findExistingByQuvirlProductId(shop, quvirlProductId);

  if (byProductId?.cj_sourcing_id || byProductId?.cj_source_number) {
    return byProductId;
  }

  const byUrl = await findExistingByProductUrl(shop, productUrl);

  if (byUrl?.cj_sourcing_id || byUrl?.cj_source_number) {
    return byUrl;
  }

  const byImageTitle = await findExistingByImageAndTitle(
    shop,
    productImage,
    productTitle
  );

  if (byImageTitle?.cj_sourcing_id || byImageTitle?.cj_source_number) {
    return byImageTitle;
  }

  return null;
}

async function saveSourcingRequest(payload) {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/cj_sourcing_requests`, {
    method: 'POST',
    headers: {
      apikey: SUPABASE_SERVICE_ROLE_KEY,
      Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
      'Content-Type': 'application/json',
      Prefer: 'return=representation'
    },
    body: JSON.stringify(payload)
  });

  const text = await response.text();

  let rows;
  try {
    rows = JSON.parse(text);
  } catch {
    rows = [];
  }

  if (!response.ok) {
    throw new Error('Failed to save CJ sourcing request: ' + text);
  }

  return Array.isArray(rows) ? rows[0] : null;
}

module.exports = async function handler(req, res) {
  try {
    if (req.method !== 'POST' && req.method !== 'GET') {
      return res.status(405).json({
        ok: false,
        error: 'Method not allowed'
      });
    }

    if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
      return res.status(500).json({
        ok: false,
        error: 'Missing Supabase environment variables'
      });
    }

    const shop = clean(getRequestValue(req, 'shop'));
    const installToken = clean(getRequestValue(req, 'installToken'));

    if (!shop || !installToken) {
      return res.status(400).json({
        ok: false,
        error: 'Missing shop or installToken'
      });
    }

    const store = await getShopifyStore(shop, installToken);

    if (!store) {
      return res.status(401).json({
        ok: false,
        error: 'Shopify store is not connected or token is invalid'
      });
    }

    const connection = await getLatestCJConnectionForShop(shop);

    if (!connection) {
      return res.status(400).json({
        ok: false,
        code: 'CJ_NOT_CONNECTED',
        error: 'CJ account is not connected for this Shopify store'
      });
    }

    const productTitle =
      clean(getRequestValue(req, 'productName')) ||
      clean(getRequestValue(req, 'title')) ||
      clean(getRequestValue(req, 'productTitle'));

    const productImage =
      safeUrl(getRequestValue(req, 'productImage')) ||
      safeUrl(getRequestValue(req, 'imageUrl')) ||
      safeUrl(getRequestValue(req, 'productImageUrl'));

    const productUrl =
      safeUrl(getRequestValue(req, 'productUrl')) ||
      safeUrl(getRequestValue(req, 'sourceUrl')) ||
      safeUrl(getRequestValue(req, 'url'));

    const price = clean(getRequestValue(req, 'price'));

    const quvirlProductId = clean(
      getRequestValue(req, 'quvirlProductId') ||
        getRequestValue(req, 'id') ||
        productUrl ||
        productTitle
    );

    if (!productTitle) {
      return res.status(400).json({
        ok: false,
        error: 'Missing product title'
      });
    }

    if (!productImage) {
      return res.status(400).json({
        ok: false,
        error: 'Missing product image URL'
      });
    }

    const existing = await findExistingSourcingRequest({
      shop,
      quvirlProductId,
      productUrl,
      productImage,
      productTitle
    });

    if (existing?.cj_sourcing_id || existing?.cj_source_number) {
      return res.status(200).json({
        ok: true,
        reused: true,
        cjSourcingId: existing.cj_sourcing_id || '',
        cjSourceNumber: existing.cj_source_number || '',
        sourcingRequestId: existing.id || null,
        sourceStatus: existing.source_status || '',
        sourceStatusText: existing.source_status_text || '',
        productTitle: existing.product_title || productTitle,
        productImageUrl: existing.product_image_url || productImage,
        productUrl: existing.product_url || productUrl,
        response: existing.raw_create_response || null,
        queryResponse: existing.raw_query_response || null
      });
    }

    const cjPayload = {
      thirdProductId: clean(getRequestValue(req, 'thirdProductId') || getRequestValue(req, 'productId')),
      thirdVariantId: clean(getRequestValue(req, 'thirdVariantId') || getRequestValue(req, 'variantId')),
      thirdProductSku: clean(getRequestValue(req, 'thirdProductSku') || getRequestValue(req, 'sku')),
      productName: productTitle,
      productImage,
      productUrl,
      remark: clean(
        getRequestValue(req, 'remark') ||
          'Created by Quvirl exact CJ supplier sourcing flow. Please source the same or closest product from this image and title.'
      ),
      price
    };

    const response = await fetch(CJ_CREATE_SOURCING_URL, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'CJ-Access-Token': connection.access_token
      },
      body: JSON.stringify(cjPayload)
    });

    const text = await response.text();

    let json;
    try {
      json = JSON.parse(text);
    } catch {
      throw new Error('CJ sourcing create response was not JSON: ' + (text || '[EMPTY RESPONSE]'));
    }

    if (!response.ok || json.success === false || json.result === false) {
      return res.status(400).json({
        ok: false,
        error: json
      });
    }

    const cjSourcingId = extractSourcingId(json);
    const cjSourceNumber = extractSourceNumber(json);

    const saved = await saveSourcingRequest({
      shop_domain: shop,
      quvirl_product_id: quvirlProductId,
      product_title: productTitle,
      product_image_url: productImage,
      product_url: productUrl,
      price,
      cj_sourcing_id: cjSourcingId ? String(cjSourcingId) : '',
      cj_source_number: cjSourceNumber ? String(cjSourceNumber) : '',
      raw_create_response: json,
      updated_at: new Date().toISOString()
    });

    return res.status(200).json({
      ok: true,
      reused: false,
      cjSourcingId,
      cjSourceNumber,
      sourcingRequestId: saved?.id || null,
      response: json
    });
  } catch (error) {
    return res.status(500).json({
      ok: false,
      error: error.message || String(error)
    });
  }
};
