  ...
  const query = `
    query getProductSupplierMetafields($id: ID!) {
      product(id: $id) {
        id
        title
        metafields(first: 50, namespace: "quvirl") {
          nodes {
            key
            value
          }
...
function detectSupplierPlatform(supplierFields) {
  const rawPlatform = clean(
    supplierFields.supplier_platform ||
      supplierFields.platform ||
      supplierFields.supplier_source ||
      supplierFields.source ||
      ''
  ).toLowerCase();

  if (
    rawPlatform === 'cjdropshipping' ||
    rawPlatform === 'cj' ||
    rawPlatform.includes('cjdropshipping')
  ) {
    return 'cjdropshipping';
  }

  if (
    supplierFields.cj_product_id ||
    supplierFields.cj_variant_id ||
    supplierFields.cj_variant_sku ||
    supplierFields.sourcing_id
  ) {
    return 'cjdropshipping';
  }
...
function getCustomerName(order) {
  return (
    clean(order.shipping_address?.name) ||
    clean(
...
function cjShippingAddressComplete(address) {
  if (!address) return false;
  ...
  return Boolean(
    address.address1 &&
      ...
      address.country_code &&
      address.zip &&
      (address.province || address.province_code || address.state)
  );
}

function fulfillmentFailureReasonAliExpress({ supplierFields, address }) {
...
function fulfillmentFailureReasonCJ({ supplierFields, address }) {
  const cjVariantId =
    supplierFields.cj_variant_id ||
    supplierFields.variant_id ||
    supplierFields.variantId ||
    supplierFields.vid ||
    '';

  if (!cjVariantId) {
    return 'missing_cj_variant_id';
  }

  if (!cjShippingAddressComplete(address)) {
    return 'missing_or_incomplete_shipping_address';
  }
...
async function findExistingAliExpressFulfillmentRecord(payload) {
  const response = await fetch(
  ...
  if (!response.ok) {
    throw new Error('Supabase AliExpress duplicate lookup failed: ' + JSON.stringify(rows));
  }

  return Array.isArray(rows) && rows.length ? rows[0] : null;
}

async function saveAliExpressFulfillmentRecord(payload) {
  const existing = await findExistingAliExpressFulfillmentRecord(payload);
  ...
  if (!response.ok) {
    ...
    if (parsedError?.code === '23505') {
      const duplicate = await findExistingAliExpressFulfillmentRecord(payload);
    ...
    throw new Error('Supabase AliExpress fulfillment save failed: ' + text);
  }
...
async function findExistingCJFulfillmentRecord(payload) {
  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/cj_auto_fulfillments?shopify_order_id=eq.${encodeURIComponent(payload.shopify_order_id)}&shopify_line_item_id=eq.${encodeURIComponent(payload.shopify_line_item_id)}&select=*`,
    {
      headers: {
        apikey: SUPABASE_SERVICE_ROLE_KEY,
        Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`
  ...
  if (!response.ok) {
    throw new Error('Supabase CJ duplicate lookup failed: ' + JSON.stringify(rows));
  }

  return Array.isArray(rows) && rows.length ? rows[0] : null;
}

async function saveCJFulfillmentRecord(payload) {
  const existing = await findExistingCJFulfillmentRecord(payload);
  ...
  const response = await fetch(`${SUPABASE_URL}/rest/v1/cj_auto_fulfillments`, {
    method: 'POST',
    headers: {
      apikey: SUPABASE_SERVICE_ROLE_KEY,
      Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
  ...
  if (!response.ok) {
    ...
    if (parsedError?.code === '23505') {
      const duplicate = await findExistingCJFulfillmentRecord(payload);
    ...
    throw new Error('Supabase CJ fulfillment save failed: ' + text);
  }
...
async function triggerCJCreateOrder(fulfillmentId) {
  const response = await fetch(`${QUVIRL_APP_URL}/api/cj/create-order`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-Quvirl-Internal-Key': QUVIRL_INTERNAL_API_KEY
...
async function processAliExpressLineItem({ shop, order, lineItem, supplierFields }) {
  const reason = fulfillmentFailureReasonAliExpress({
    supplierFields,
  ...
  const saved = await saveAliExpressFulfillmentRecord({
    shop_domain: shop,
  ...
  if (saved.existed) {
    await saveWebhookDebugLog({
      ...
      hmac_present: true,
      hmac_valid: true,
      status: 'duplicate_aliexpress_fulfillment_skipped',
      error: `Existing AliExpress fulfillment record found for order ${order.name || order.id}, line item ${lineItem.id || ''}`
    });
  ...
  if (!reason && record?.id) {
    ...
    await saveWebhookDebugLog({
      ...
      hmac_present: true,
      hmac_valid: true,
      status: 'aliexpress_fulfillment_record_saved_with_failure_reason',
      error: reason || null
    });
  }
}

async function processCJLineItem({ shop, order, lineItem, supplierFields }) {
  const reason = fulfillmentFailureReasonCJ({
    supplierFields,
    address: order.shipping_address
  });
  ...
  const cjProductId =
    supplierFields.cj_product_id ||
    supplierFields.product_id ||
    supplierFields.productId ||
    supplierFields.pid ||
    '';
  ...
  const saved = await saveCJFulfillmentRecord({
    shop_domain: shop,
    shopify_order_id: String(order.id),
    shopify_order_name: String(order.name || ''),
    shopify_line_item_id: String(lineItem.id || ''),

    supplier_platform: 'cjdropshipping',
    cj_product_id: cjProductId,
    cj_variant_id: cjVariantId,
    cj_variant_sku:
      supplierFields.cj_variant_sku ||
      supplierFields.variant_sku ||
      supplierFields.variantSku ||
      '',
    sourcing_id: supplierFields.sourcing_id || supplierFields.cj_sourcing_id || '',
  ...
  if (saved.existed) {
    await saveWebhookDebugLog({
      ...
      hmac_present: true,
      hmac_valid: true,
      status: 'duplicate_cj_fulfillment_skipped',
      error: `Existing CJ fulfillment record found for order ${order.name || order.id}, line item ${lineItem.id || ''}`
    });
  ...
  if (!reason && record?.id) {
    const cjResult = await triggerCJCreateOrder(record.id);
    ...
    await saveWebhookDebugLog({
      ...
      hmac_valid: true,
      status: cjResult?.ok
        ? 'cj_create_order_triggered'
        : 'cj_create_order_failed',
      error: cjResult?.ok ? null : JSON.stringify(cjResult)
    });
  } else {
    await saveWebhookDebugLog({
      shop_domain: shop,
      ...
      hmac_present: true,
      hmac_valid: true,
      status: 'cj_fulfillment_record_saved_with_failure_reason',
      error: reason || null
    });
  }
}
...
async function processLineItem({ shop, store, order, lineItem }) {
  ...
  const supplierPlatform = detectSupplierPlatform(supplierFields);

  if (supplierPlatform === 'cjdropshipping') {
    await processCJLineItem({
      shop,
      order,
      lineItem,
      supplierFields
  ...
  await processAliExpressLineItem({
    shop,
    order,
    lineItem,
    supplierFields
