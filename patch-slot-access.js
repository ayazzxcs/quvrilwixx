...
const SLOT_ENABLED =
  process.env.ENABLE_SLOT_RESERVATION === 'true';

const SCRIPT = '/slot-access.jsscript>';
...
function patchFile(file) {
  ...
  html = html.replace(
    /<script\s+src=["']\/slot-access\.js["']\s+defer><\/script>\s*/g,
    ''
  );
...
console.log(
  `Slot reservation ${SLOT_ENABLED ? 'ENABLED' : 'DISABLED'} on ${count} pages.`
);
