...
function resolveTrendDirection(value, direction) {
  const n = Math.round(num(value));
  const d = String(direction || '').trim().toLowerCase();
  if (n > 0) return 'up';
  if (n < 0) return 'down';
  if (['up', 'rising', 'rise', 'increasing', 'increase', 'positive'].includes(d)) return 'up';
  if (['down', 'falling', 'declining', 'decline', 'decreasing', 'decrease', 'negative'].includes(d)) return 'down';
  return 'flat';
}
...
function supplierData(raw) {
  const pr = proof(raw) || {};
  return pr.supplierSource || pr.cjSupplier || raw?.supplierSource || {};
}
function supplierNameFrom(value) {
  const s = String(value || '').trim();
  if (/ali\s*express/i.test(s)) return 'AliExpress';
  if (/cj\s*dropshipping|cjdropshipping|\bcj\b/i.test(s)) return 'CJdropshipping';
  return s || 'Supplier';
}
function supplierName(p) {
  const ss = supplierData(p);
  return supplierNameFrom(first(p?.supplier, p?.source, p?.marketplace, ss.supplier, ss.source, p?.raw?.source, 'Supplier'));
}
function supplierShortName(p) {
  const n = supplierName(p);
  if (/ali/i.test(n)) return 'AliExpress';
  if (/cj/i.test(n)) return 'CJ';
  return 'Supplier';
}
function supplierLinkText(p) { return `${supplierShortName(p)} Supplier`; }
...
function normalize(raw) {
  ...
  const ss = supplierData(raw);
  const supplier = supplierName(raw);
  ...
  const image = first(raw.image, raw.productImage, raw.bigImage, raw.img, raw.raw?.productImage, raw.raw?.bigImage, raw.raw?.image, Array.isArray(raw.raw?.productImageSet) ? raw.raw.productImageSet[0] : '');
  const supplierUrl = first(raw.supplierUrl, raw.productUrl, raw.productLink, raw.shopUrl, raw.raw?.url, raw.raw?.productUrl, raw.raw?.productLink, ss.productUrl, 'https://www.cjdropshipping.com/');
  const cost = num(first(raw.cost, raw.supplierPrice, raw.sellPrice, raw.productPrice, raw.raw?.price, raw.raw?.sellPrice, raw.raw?.productPrice, ss.price, cj.price));
  const shipping = num(first(raw.shipping, raw.shippingPrice, raw.freight, raw.raw?.shippingPrice, ss.shipping, cj.shipping));
  ...
  const googleGrowth = num(first(google.growthPercent, raw.growthPercent, raw.googleGrowth));
  const trendDirection = resolveTrendDirection(
    googleGrowth,
    first(raw.trendDirection, raw.googleTrendDirection, google.trendDirection, '')
  );
  return {
    ...raw,
    id, name, category, image, supplierUrl, supplier, cost, shipping, sell, profit, margin, score,
    googleGrowth,
    ...
    cjScore: num(first(ss.score, cj.score, raw.supplierScore, raw.cjScore, raw.cjSupplierScore)),
...
function normalizeTimelinePoint(point, index) {
  if (typeof point === 'number') return { label: `P${index + 1}`, value: point };
  const value = num(first(point?.value, point?.score, point?.interest, point?.y, point));
  const label = first(point?.label, point?.date, point?.time, `P${index + 1}`);
  return { label: String(label), value };
}
function scoreHistory(p) {
  const h = Array.isArray(p.scoreHistory) ? p.scoreHistory : (Array.isArray(p.history) ? p.history : []);
  return h.filter(Boolean).slice(-370);
}
function googleTimeline(p) {
  const gt = proof(p).googleTrends || {};
  let points = [];
  if (Array.isArray(gt.timeline)) points = gt.timeline.map(normalizeTimelinePoint);
  else if (Array.isArray(gt.timelineValues)) points = gt.timelineValues.map(normalizeTimelinePoint);
  points = points.filter(x => Number.isFinite(x.value));
  if (points.length) return points.slice(-60);
  return scoreHistory(p).map((h, i) => ({ label: first(h.date, `Run ${i + 1}`), value: num(first(h.googleScore, h.googleTrendScore, 0)) })).filter(x => x.value > 0).slice(-60);
}
function lineChart(points, { valueKey = 'value', suffix = '' } = {}) {
  const clean = points.map((p, i) => ({ label: String(p.label || `P${i + 1}`), value: num(p[valueKey] ?? p.value) })).filter(p => Number.isFinite(p.value));
  if (clean.length < 2) return `<div class="chartWrap muted">Not enough history yet. This chart will grow after future Quvirl runs.</div>`;
  const w = 720, h = 220, pad = 28;
  const vals = clean.map(p => p.value);
  const min = Math.min(0, ...vals), max = Math.max(100, ...vals, 1);
  const x = i => pad + (i * (w - pad * 2)) / (clean.length - 1);
  const y = v => h - pad - ((v - min) / (max - min)) * (h - pad * 2);
  const d = clean.map((p, i) => `${i ? 'L' : 'M'}${x(i).toFixed(1)} ${y(p.value).toFixed(1)}`).join(' ');
  const area = `${d} L${x(clean.length - 1).toFixed(1)} ${h - pad} L${x(0).toFixed(1)} ${h - pad} Z`;
  const every = Math.ceil(clean.length / 8);
  const dots = clean.filter((_, i) => i === 0 || i === clean.length - 1 || i % every === 0).map(point => {
    const idx = clean.indexOf(point);
    return `<circle class="chartDot" cx="${x(idx).toFixed(1)}" cy="${y(point.value).toFixed(1)}" r="3"><title>${esc(point.label)}: ${Math.round(point.value)}${suffix}</title></circle>`;
  }).join('');
  return `<div class="chartWrap"><svg class="chartSvg" viewBox="0 0 ${w} ${h}" role="img" aria-label="Trend timeline chart"><line class="chartGrid" x1="${pad}" y1="${pad}" x2="${w - pad}" y2="${pad}"/><line class="chartGrid" x1="${pad}" y1="${h / 2}" x2="${w - pad}" y2="${h / 2}"/><line class="chartGrid" x1="${pad}" y1="${h - pad}" x2="${w - pad}" y2="${h - pad}"/><path class="chartArea" d="${area}"/><path class="chartLine" d="${d}"/>${dots}</svg><div class="chartLabels"><span>${esc(clean[0].label)}</span><span>${esc(clean[clean.length - 1].label)}</span></div></div>`;
}
function trendTimelineSection(p) {
  const gt = proof(p).googleTrends || {};
  const timeline = googleTimeline(p);
  const history = scoreHistory(p);
  const latest = timeline.length ? timeline[timeline.length - 1].value : 0;
  const firstValue = timeline.length ? timeline[0].value : 0;
  const change = timeline.length > 1 ? Math.round(latest - firstValue) : 0;
  const historyChart = history.map(h => ({ label: first(h.date, 'Run'), value: num(first(h.quvirlScore, h.dropTrendScore, h.score)) })).filter(x => x.value > 0);
  return `<section class="trendTimeline"><h2>Google Trends timeline and score history</h2><p class="muted">This chart is saved directly from Quvirl's Google Trends run for <b>${esc(first(gt.usedKeyword, gt.keyword, p.name))}</b>. It helps show whether demand is stable, rising, or only a short spike.</p>${lineChart(timeline)}<div class="historyMetrics"><div class="metric"><span class="muted">Timeline points</span><br><b>${timeline.length || '—'}</b></div><div class="metric"><span class="muted">Latest interest</span><br><b>${timeline.length ? Math.round(latest) : '—'}/100</b></div><div class="metric"><span class="muted">Start-to-latest change</span><br><b>${timeline.length > 1 ? (change >= 0 ? '+' : '') + change : '—'}</b></div><div class="metric"><span class="muted">Saved score runs</span><br><b>${history.length}</b></div></div>${historyChart.length > 1 ? `<h3 style="margin-top:18px">Quvirl score history</h3>${lineChart(historyChart, { suffix: '/100' })}` : `<p class="muted" style="margin-top:12px">Quvirl score history will appear after this product is saved across multiple runs.</p>`}</section>`;
}

function formatGrowth(n, direction) {
  n = Math.round(num(n));
  const resolved = resolveTrendDirection(n, direction);
  if (resolved === 'up') return n ? `Google searches increased about ${Math.abs(n)}%` : 'Google searches are rising';
  if (resolved === 'down') return n ? `Google searches decreased about ${Math.abs(n)}%` : 'Google searches are declining';
  return 'Google interest is currently stable';
}
...
function signalBars(p) {
  ...
  return `<div class="signalBars"><div class="signalRow"><span>Google</span><div class="signalBar"><i style="width:${g}%"></i></div><b>${g || '—'}</b></div><div class="signalRow"><span>Amazon</span><div class="signalBar"><i style="width:${a}%"></i></div><b>${a || '—'}</b></div><div class="signalRow"><span>Supplier</span><div class="signalBar"><i style="width:${c}%"></i></div><b>${c || '—'}</b></div></div>`;
}
function visualChips(p) {
  const direction = resolveTrendDirection(p.googleGrowth, p.trendDirection);
  const chips = [
    direction === 'up' ? 'Rising search' : direction === 'down' ? 'Declining search' : 'Stable search',
    p.amazonScore >= 50 ? 'Amazon demand' : 'Demand check',
    p.margin >= 50 ? 'High margin' : 'Margin ready',
    p.cjScore >= 50 ? `${supplierShortName(p)} ready` : `${supplierShortName(p)} check`
  ];
...
function layout({ title, description, canonical, body, schema = [], extraHead = '' }) {
  const schemas = [ORGANIZATION_SCHEMA, ...(Array.isArray(schema) ? schema.filter(Boolean) : [schema].filter(Boolean))];
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
function productPage(p, all) {
  ...
  const desc = `${p.name} trend research with Quvirl score ${p.score}/100, Google Trends movement, Amazon demand, supplier checks, audience ideas, ad angles, margin notes and risk analysis.`;
  const rel = relatedProducts(p, all, 9);
  const risks = riskNotes(p);
  const hooks = adHooks(p);
  const audience = audienceProfile(p);
  const faq = [
    { q: `Is ${p.name} good for dropshipping?`, a: `${p.name} may be worth researching because Quvirl found a mix of monthly Google Trends, Amazon demand and supplier signals from CJdropshipping/AliExpress. It should still be validated with supplier checks and a small test.` },
    ...
    { q: 'What does the Quvirl score mean?', a: 'The Quvirl score combines Google Trends, Amazon demand and supplier metrics into one monthly product research score. It is a shortlisting signal, not a sales guarantee.' },
  ...
  const schema = [
    {'@context':'https://schema.org','@type':'BreadcrumbList','itemListElement':[{'@type':'ListItem','position':1,'name':'Quvirl','item':SITE_URL+'/'},{'@type':'ListItem','position':2,'name':p.category,'item':`${SITE_URL}/category/${categorySlug(p.category)}/`},{'@type':'ListItem','position':3,'name':p.name,'item':canonical}]},
    {'@context':'https://schema.org','@type':'Product','name':p.name,'image':p.image?[p.image]:undefined,'category':p.category,'description':desc,'brand':{'@type':'Brand','name':'Quvirl'},'offers':{'@type':'Offer','priceCurrency':'USD','price':p.sell||p.cost||0,'availability':'https://schema.org/InStock','url':canonical},...(p.amazonRating?{'aggregateRating':{'@type':'AggregateRating','ratingValue':String(p.amazonRating),'reviewCount':String(p.amazonReviews||1)}}:{})},
    {'@context':'https://schema.org','@type':'FAQPage','mainEntity':faq.map(f=>({'@type':'Question','name':f.q,'acceptedAnswer':{'@type':'Answer','text':f.a}}))}
  ];
  const body = `<main>
<section class="hero"><div class="two"><div><img src="${esc(p.image)}" alt="${esc(p.name)} product image" style="width:100%;border-radius:18px;background:#fff" loading="eager"></div><div><span class="pill">${esc(p.category)}</span><span class="pill">Confidence: ${esc(p.confidence)}</span><h1>${esc(p.name)}</h1><p class="muted">Dropshipping product research powered by monthly Google Trends, Amazon market demand and supplier data.</p><div style="display:flex;align-items:center;gap:14px;margin:14px 0"><div class="scoreRing" style="--score:${clampScore(p.score)}"><b>${clampScore(p.score)}</b></div><div><b>Quvirl Score</b><p class="muted" style="margin:3px 0 0">Combined market intelligence score out of 100.</p></div></div>${visualChips(p)}${signalBars(p)}<div class="metric">Google Trends: <b>${esc(formatGrowth(p.googleGrowth, p.trendDirection))}</b></div><div class="metric">Amazon Demand: <b>${p.amazonScore || 0}/100</b>${p.amazonReviews ? ` • ${p.amazonReviews.toLocaleString()} review signals` : ''}${p.amazonRating ? ` • ${esc(p.amazonRating)}★` : ''}</div><div class="metric">Supplier Score: <b>${p.cjScore || 0}/100</b></div><div class="metric">Estimated Profit: <b>${money(p.profit)}</b> • Margin: <b>${p.margin}%</b></div><p class="links"><a href="${esc(p.supplierUrl)}" target="_blank" rel="noopener">${esc(supplierLinkText(p))}</a><a class="btn2" href="/category/${categorySlug(p.category)}/">More ${esc(p.category)}</a></p></div></div></section>
...
${trendTimelineSection(p)}
<section class="card" style="margin-top:18px"><h2>Why this product is trending</h2><p>${esc(p.name)} appears on Quvirl because it matched a combination of recent monthly real-market product signals. This page checks search momentum from Google Trends, marketplace validation from Amazon demand signals, and supplier readiness from CJdropshipping/AliExpress data.</p><p>For sellers, this means ${esc(p.name)} can be reviewed as a possible product test instead of guessing from random viral videos. The strongest parts to check are the Quvirl score, category fit, margin, supplier image quality, review/rating signal and whether the product solves a clear buyer problem.</p><p>This product belongs to the ${esc(p.category)} market, so the best test is usually a focused landing page, one clear product benefit, and 3–5 short ad creatives. Avoid treating the score as a guaranteed winner. Use it as a shortlist signal, then validate buyer response with small-budget testing, organic TikTok/Reels posts and supplier delivery checks.</p></section>
...
<section class="card" style="margin-top:18px"><h2>Monthly market research summary</h2><p><b>Search demand:</b> ${esc(formatGrowth(p.googleGrowth, p.trendDirection))}. A stable or rising monthly pattern is usually safer for dropshippers than a short weekly spike because product sourcing, page setup and ad testing take time.</p><p><b>Marketplace validation:</b> Amazon demand score is ${p.amazonScore || 0}/100${p.amazonReviews ? ` with ${p.amazonReviews.toLocaleString()} review signals` : ''}. This helps identify whether buyers already understand the product type.</p><p><b>Supplier readiness:</b> ${esc(supplierName(p))} supplier score is ${p.cjScore || 0}/100. Before testing, still check processing time, shipping methods, product variants, image quality, refund risk and whether the supplier page looks active.</p></section>
...
function listingPage({ slug, title, description, intro, products, priority = '0.8' }) {
  ...
  const body = `<section class="hero"><h1>${esc(title)}</h1><p class="muted">${esc(intro)}</p><p class="pill">Monthly market signal update: ${esc(MONTH_KEY)}</p><div class="toc"><a href="/top-trending-products/">Top trending products</a><a href="/trending-this-month/">Trending this month</a><a href="/category/pet-products/">Pet products</a><a href="/category/home-living-products/">Home products</a><a href="/category/beauty-products/">Beauty products</a><a href="/blog/best-dropshipping-products-2026/">Best products 2026</a></div></section><div class="grid">${products.map(productCard).join('')}</div><section class="card" style="margin-top:24px"><h2>How Quvirl ranks these products</h2><p>Quvirl ranks products using recent monthly Google Trends search demand, Amazon market demand and supplier data from CJdropshipping/AliExpress. Use this page to shortlist products, then check supplier quality, ad creatives, delivery time and margin before testing.</p><p class="muted">This page is designed for dropshippers who need more stable demand signals, not short-lived weekly hype.</p></section>`;
...
function blogIndexPage(products) {
  ...
  const posts = [
    ...
    { href:'/blog/cjdropshipping-supplier-checklist/', title:'CJ + AliExpress Supplier Checklist', desc:'Review supplier images, variants, stock, shipping and source links before launch.' }
  ];
  const body = `<section class="hero"><h1>Quvirl Product Research Blog</h1><p class="muted">Original dropshipping research guides based on Google Trends, Amazon demand, supplier data, margin planning and product-page SEO.</p><div class="grid">${posts.map(post => `<article class="card"><h2><a href="${post.href}">${esc(post.title)}</a></h2><p class="muted">${esc(post.desc)}</p><a class="btn" href="${post.href}">Read guide</a></article>`).join('')}</div></section><section style="margin-top:24px"><h2>Popular products from this month</h2><div class="grid">${top.map(productCard).join('')}</div></section>`;
  const schema = {'@context':'https://schema.org','@type':'Blog','name':'Quvirl Product Research Blog','description':'Dropshipping product research guides using monthly market signals.','url':canonical};
  return { html: layout({ title:'Quvirl Blog - Dropshipping Product Research', description:'Original dropshipping product research guides based on Google Trends, Amazon demand, margin planning and supplier data.', canonical, body, schema }), url: canonical, priority: '0.8' };
}
...
async function main() {
  // Remove stale generated product/category pages so old URLs do not keep outdated CJ-only copy.
  for (const dir of ['product', 'category']) fs.rmSync(dir, { recursive: true, force: true });
  ...
  pages.push(listingPage({ slug:'high-margin-products', title:'High Margin Dropshipping Products', description:'High-margin dropshipping product ideas sorted by estimated profit margin, monthly demand signals and supplier data.', intro:'A monthly profit-focused research page for sellers who want product ideas with better room for ad costs and testing.', products:[...products].sort((a,b)=>b.margin-a.margin || b.profit-a.profit || b.score-a.score).slice(0,120), priority:'0.84' }));
  pages.push(listingPage({ slug:'viral-tiktok-products', title:'Viral TikTok Products for Dropshipping', description:'TikTok-friendly product ideas ranked by visual appeal, product demand, monthly trend signals and supplier readiness.', intro:'Use this monthly shortlist to find products that can work for TikTok, Reels, UGC demos and short-form product ads.', products:products.filter(p => !/retainer|replacement|bumper|bracket|clip|fastener|auto part/i.test((p.name||'') + ' ' + (p.category||''))).slice(0,120), priority:'0.84' }));
  ...
  for (const [key, list] of groups) {
    ...
    pages.push(listingPage({ slug:`category/${key}`, title:`Trending ${nice} for Dropshipping`, description:`Find trending ${nice.toLowerCase()} ranked by Quvirl score, Google Trends, Amazon demand and supplier data.`, intro:`Browse ${nice.toLowerCase()} with product images, supplier links, trend signals and margin estimates.`, products:list.slice(0,80), priority:'0.85' }));
  }
  pages.push(blogIndexPage(products));
  pages.push(blogPage('best-dropshipping-products-2026', 'Best Dropshipping Products 2026', 'A monthly Quvirl guide to product ideas using Google Trends, Amazon demand and supplier data.', products, 'Top product ideas to review'));
