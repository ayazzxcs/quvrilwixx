(function () {
  ...
  function createStyles() {
    ...
    style.textContent = `
      .qv-shopify-floating{position:fixed;right:18px;bottom:86px;z-index:99999;display:flex;flex-direction:column;gap:8px;align-items:flex-end}
      .qv-shopify-btn{border:1px solid rgba(148,163,184,.25);border-radius:999px;padding:12px 16px;font-size:14px;font-weight:900;cursor:pointer;box-shadow:0 14px 40px rgba(0,0,0,.35)}
      .qv-shopify-btn:active{transform:scale(.98)}
      .qv-shopify-btn:disabled{opacity:.65;cursor:not-allowed}
      .qv-shopify-connect{background:#0d1117;color:#f0f6fc;border-color:#30363d}
      .qv-shopify-disconnect{background:rgba(127,29,29,.92);color:#fff;border-color:rgba(248,113,113,.5)}
      .qv-shopify-add{background:linear-gradient(135deg,#22c55e,#14b8a6);color:#001b0b;border-color:rgba(52,211,153,.8)}
      .qv-ali-connect{background:linear-gradient(135deg,#ff7a18,#ff3d00);color:#fff;border-color:rgba(251,146,60,.75)}
      .qv-overlay{position:fixed;inset:0;z-index:100000;display:none;align-items:center;justify-content:center;padding:18px;background:rgba(2,6,23,.72);backdrop-filter:blur(10px)}
      .qv-overlay.qv-open{display:flex}
      .qv-modal{width:min(94vw,820px);max-height:88vh;overflow:auto;border:1px solid rgba(148,163,184,.22);border-radius:28px;padding:22px;background:radial-gradient(circle at top left,rgba(34,197,94,.20),transparent 34%),linear-gradient(145deg,#0f172a,#020617);color:#e5eefb;box-shadow:0 30px 90px rgba(0,0,0,.55)}
      .qv-kicker{display:inline-flex;margin-bottom:12px;padding:7px 11px;border-radius:999px;background:rgba(34,197,94,.12);border:1px solid rgba(34,197,94,.28);color:#86efac;font-size:12px;font-weight:900;text-transform:uppercase}
      .qv-title{margin:0 0 8px;font-size:26px;line-height:1.07;font-weight:950;color:#fff}
      ...
      .qv-input{width:100%;box-sizing:border-box;border-radius:16px;border:1px solid rgba(148,163,184,.22);background:rgba(15,23,42,.9);color:#fff;padding:14px;outline:none;font-size:15px;margin-bottom:12px}
      .qv-actions{display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-end;margin-top:18px}
      .qv-action{border:1px solid rgba(148,163,184,.25);border-radius:14px;padding:12px 14px;font-size:14px;font-weight:900;cursor:pointer}
      .qv-secondary{background:rgba(15,23,42,.85);color:#e2e8f0}
      .qv-primary{background:linear-gradient(135deg,#22c55e,#14b8a6);color:#001b0b}
      .qv-status{margin-top:12px;padding:10px 12px;border-radius:14px;background:rgba(15,23,42,.72);border:1px solid rgba(148,163,184,.16);color:#a7f3d0;font-size:13px;line-height:1.45;display:none}
      .qv-status.qv-show{display:block}
      .qv-supplier-tabs{display:flex;flex-wrap:wrap;gap:8px;margin:14px 0 12px}
      .qv-supplier-tab{border:1px solid rgba(148,163,184,.25);border-radius:999px;padding:9px 12px;background:rgba(15,23,42,.8);color:#dbeafe;font-size:13px;font-weight:900;cursor:pointer}
      .qv-supplier-tab.qv-active{background:linear-gradient(135deg,#22c55e,#14b8a6);color:#001b0b}
      .qv-supplier-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(215px,1fr));gap:12px;margin-top:16px}
      .qv-supplier-card{border:1px solid rgba(148,163,184,.18);border-radius:18px;overflow:hidden;background:rgba(15,23,42,.82)}
      ...
      .qv-supplier-title{margin:0 0 8px;color:#fff;font-size:13px;line-height:1.35;font-weight:800}
      .qv-supplier-meta{color:#a7f3d0;font-size:12px;line-height:1.45;margin-bottom:10px}
      .qv-supplier-badge{display:inline-flex;margin-bottom:8px;padding:5px 8px;border-radius:999px;font-size:11px;font-weight:950;background:rgba(59,130,246,.18);color:#bfdbfe}
      .qv-supplier-badge.qv-cj{background:rgba(250,204,21,.13);color:#fef08a}
      .qv-match{margin:0 0 10px;padding:8px 9px;border-radius:12px;background:rgba(15,23,42,.72);border:1px solid rgba(148,163,184,.16);color:#dbeafe;font-size:11px;line-height:1.45}
      .qv-match strong{color:#a7f3d0}
      .qv-supplier-select{width:100%;border:1px solid rgba(52,211,153,.55);border-radius:12px;background:linear-gradient(135deg,#22c55e,#14b8a6);color:#001b0b;padding:10px 11px;font-weight:900;cursor:pointer}
      .qv-supplier-select:disabled{opacity:.65;cursor:not-allowed}
  ...
  function renderCJSourcingSubmitted(result, product) {
    ...
    if (checkButton) {
      checkButton.addEventListener('click', async function () {
        ...
        checkButton.textContent = 'Checking CJ sourcing result...';

        try {
          await checkCJSourcingResult(
          ...
          setTimeout(function () {
            ...
            checkButton.textContent = 'Check CJ sourcing result';
          }, 1800);
        }
      });
    }
  ...
  async function checkCJSourcingResult(cjSourcingId, cjSourceNumber = '') {
    ...
    try {
      ...
      if (!response.ok || !result.ok) {
        if (response.status === 429 || result.code === 'CJ_RATE_LIMITED') {
          status.textContent =
            'CJ rate limit reached. Please wait a few seconds and try again.';
          return;
        }
